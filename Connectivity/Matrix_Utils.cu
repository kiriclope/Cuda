...
__host__ void CreatePath(char *&path) {
  ...
  char* cdum ;
  cdum = (char *) malloc(100 * sizeof(char) ) ;

  char* strCrec ;
  strCrec = (char *) malloc(100 * sizeof(char)) ;

  char* strAuta ;
  strAuta = (char *) malloc(100 * sizeof(char)) ;
  ...
  if(IF_RING) 
    if(IF_SPEC)
      ...
      if(IF_LARGE)
	sprintf(cdum, "../../Connectivity/%dpop/Large/N%d/K%.0f/Ring/%s", nbpop, (int) (N_NEURONS/nbPref), K, strCrec) ;
      else
	if(IF_AUTA)
	  sprintf(cdum, "../../Connectivity/%dpop/N%d/K%.0f/Ring/%s/AUTA/Pb%.3f", nbpop, (int) (N_NEURONS/nbPref), K, strCrec, AUTA_Pb) ;
	else
	...
	if(IF_LARGE)
	  sprintf(cdum, "../../Connectivity/%dpop/Large/N%d/K%.0f/Gauss/%s", nbpop, (int) (N_NEURONS/nbPref), K, strCrec) ;
	else
	  if(DIMENSION==1)
      ...
      if(IF_LARGE)
	sprintf(cdum, "../../Connectivity/%dpop/Large/N%d/K%.0f", nbpop, (int) (N_NEURONS/nbPref), K) ;
      else 
	if(IF_AUTA)
  ...
  path = (char *) malloc( (strlen(cdum) + 100) * sizeof(char) ) ;
  strcpy(path,cdum) ;

  char *mkdirp ;   
  mkdirp = (char *) malloc( (strlen(path) + 100) * sizeof(char) );
...
__global__ void kernelGenSparseRep(float *dev_conVec, unsigned long *dev_IdPost, int *dev_nbPost, int lChunk, unsigned long maxNeurons) {
  ...
  if(id < maxNeurons & kNeuron < N_NEURONS) {
	...
	dev_IdPost[nbPost + kNeuron * N_NEURONS] = i ; 
	nbPost += 1 ; 
      } 
